...
bind-key    -T prefix  <       display-menu -T '#[align=absolute-centre,bg=#{@thm_accent-color},fg=black] #{window_index}: #{window_name} ' -x W -y W '#{?#{>:#{session_windows},1},,-}#[fg=blue]Swap Left' l { swap-window -t :-1 } '#{?#{>:#{session_windows},1},,-}#[fg=blue]Swap Right' r { swap-window -t :+1 } '#{?pane_marked_set,,-}#[fg=blue]Swap Marked' s { swap-window } '' '#[fg=red]Kill' X { kill-window } '#[fg=blue]Respawn' R { respawn-window -k } '#{?pane_marked,#[fg=red]Unmark,#[fg=green]Mark}' m { select-pane -m } '#[fg=blue]Rename' n { command-prompt -F -I '#W' { rename-window -t '#{window_id}' '%%' } } '' '#[fg=blue]New After' w { new-window -a } '#[fg=blue]New At End' W { new-window }
bind-key    -T prefix  >       display-menu -T '#[align=absolute-centre,bg=#{@thm_accent-color},fg=black] Pane #{pane_index} ' -x P -y P '#{?#{m/r:(copy|view)-mode,#{pane_mode}},#[fg=blue]Go To Top,}' < { send-keys -X history-top } '#{?#{m/r:(copy|view)-mode,#{pane_mode}},#[fg=blue]Go To Bottom,}' > { send-keys -X history-bottom } '' '#{?mouse_word,Search For #[underscore]#{=/9/...:mouse_word},}' C-r { if-shell -F '#{?#{m/r:(copy|view)-mode,#{pane_mode}},0,1}' 'copy-mode -t=' ; send-keys -X -t = search-backward -- '#{q:mouse_word}' } '#{?mouse_word,Type #[underscore]#{=/9/...:mouse_word},}' C-y { copy-mode -q ; send-keys -l '#{q:mouse_word}' } '#{?mouse_word,Copy #[underscore]#{=/9/...:mouse_word},}' c { copy-mode -q ; set-buffer '#{q:mouse_word}' } '#{?mouse_line,Copy Line,}' l { copy-mode -q ; set-buffer '#{q:mouse_line}' } '' '#{?mouse_hyperlink,Type #[underscore]#{=/9/...:mouse_hyperlink},}' C-h { copy-mode -q ; send-keys -l '#{q:mouse_hyperlink}' } '#{?mouse_hyperlink,Copy #[underscore]#{=/9/...:mouse_hyperlink},}' h { copy-mode -q ; set-buffer '#{q:mouse_hyperlink}' } '' '#[fg=blue]Horizontal Split' h { split-window -h } '#[fg=blue]Vertical Split' v { split-window -v } '' '#{?#{>:#{window_panes},1},,-}#[fg=blue]Swap Up' u { swap-pane -U } '#{?#{>:#{window_panes},1},,-}#[fg=blue]Swap Down' d { swap-pane -D } '#{?pane_marked_set,,-}#[fg=blue]Swap Marked' s { swap-pane } '' '#[fg=red]Kill' X { kill-pane } '#[fg=blue]Respawn' R { respawn-pane -k } '#{?pane_marked,#[fg=red]Unmark,#[fg=green]Mark}' m { select-pane -m } '#{?#{>:#{window_panes},1},,-}#{?window_zoomed_flag,#[fg=yellow]Unzoom,#[fg=yellow]Zoom}' z { resize-pane -Z }
